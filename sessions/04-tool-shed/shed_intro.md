...
Main Tool Shed (MTS) runs at https://toolshed.g2.bx.psu.edu and serves all Galaxies worldwide.

Everybody can create a repository.

Every repository is public including its history.
...
* `wrapper` or `tool definition file` - The XML file that describes how the underlying software works, allowing Galaxy to render UI and execute the software in the right manner.
--

* `repository` - A versioned code archive with tool(s) in Tool Shed. Mercurial is used.
--

* Every repository update generates a new `revision`. Only reproducibility-affecting changes generate a new `installable revision` (change of version).
--

* `metadata` - TS generates and stores a set of data for every installable revision of the repository.

???
Every TS repo update can be propagated to the Galaxy.
...
* Tool development repository (often GitHub) can be linked to from the TS repository.
--

* Tool Shed allows administrators to install *old* installable revisions.
--

* *Multiple* installable revisions of any repository can be present in Galaxy. Including local tools.
...
```shell
.
├── .shed.yml # a config file for Planemo to simplify repo handling
├── macros.xml # defines xml macros usable in other xml files
├── seqtk_trimfq.xml # wrapper for tool
└── test-data
...
---
## A word about legacy TS-supplied dependencies

Repository that wants its `<requirements>` resolved via TS dependencies includes the `tool_dependencies.xml` file in root.
...
A TS recipe how to install the dependency (contents of `package_seqtk_1_2` repository).
```xml
<?xml version="1.0"?>
<tool_dependency>
